...
const ProductDetails = () => {
  ...
  const similarProducts = useMemo(() => {
    return content?.products?.filter((item) => (item?.type_id === product?.type_id && item?.id !== product?.id && item?.collection === product?.collection));
  }, [product]);
    ...
    <>
      <div className='flex flex-col md:flex-row px-10'>
              ...
              <div className='flex flex-row md:flex-col justify-center h-full'>
                {
                  (product?.thumbnail || product?.images?.length) && (
                    <>
                      {product?.thumbnail && (
                        <button onClick={() => setImage(product.thumbnail)} className='rounded-lg w-fit p-2 mb-2'>
                          <img
                            src={product.thumbnail}
                            className='h-[60px] w-[75px] rounded-lg bg-cover bg-center hover:scale-105 hover:border'
                            alt={'thumbnail'}
                          />
                        </button>
                      )}
        ...
        <div className='w-[60%] px-10'>
          ...
          {/* Price Tag */}
          <p className='text-xl bold py-3'>{product?.price} BYN</p>
          <div className='flex flex-col py-2'>
            <div className='flex gap-2'>
              <p className='text-sm bold'>Коллекция</p>
              <Link className='text-sm text-gray-500 hover:text-gray-900' to={'https://en.wikipedia.org/wiki/Clothing_sizes'} target='_blank'>{'Все ->'}</Link>
            </div>
          </div>
          {/*<div className='mt-2'><SizeFilter sizes={product?.size} hidleTitle /></div>*/}
          <div>
            <p className='text-l bold pb-2'>{product?.collection}</p>
          </div>
          <div className='flex py-4'>
            ...
            </svg>В корзину</div></button> : <button></button>}
          </div>
      ...
      <div className='flex px-10'>
        ...
        <div className='pt-4 grid grid-cols-1 lg:grid-cols-4 md:grid-cols-3 gap-8 px-2 pb-10'>
          ...
          {!similarProducts?.length && <p>Товары не найдены!</p>}
        </div>
      </div>
    </>
  )
